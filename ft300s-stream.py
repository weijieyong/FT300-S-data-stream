...
### add argument parsing for serial port
parser = argparse.ArgumentParser(description="FT300 streaming example")
parser.add_argument(
    "-p", "--port", default="/dev/ttyUSB0", help="Serial port where FT300 is connected"
)
...
def forceFromSerialMessage(serialMessage, zeroRef=[0, 0, 0, 0, 0, 0]):
    ...
    forceTorque[0] = round(
        int.from_bytes(serialMessage[2:4], byteorder="little", signed=True) / 100
        - zeroRef[0],
        2,
    )
    forceTorque[1] = round(
        int.from_bytes(serialMessage[4:6], byteorder="little", signed=True) / 100
        - zeroRef[1],
        2,
    )
    forceTorque[2] = round(
        int.from_bytes(serialMessage[6:8], byteorder="little", signed=True) / 100
        - zeroRef[2],
        2,
    )
    forceTorque[3] = round(
        int.from_bytes(serialMessage[8:10], byteorder="little", signed=True) / 1000
        - zeroRef[3],
        2,
    )
    forceTorque[4] = round(
        int.from_bytes(serialMessage[10:12], byteorder="little", signed=True) / 1000
        - zeroRef[4],
        2,
    )
    forceTorque[5] = round(
        int.from_bytes(serialMessage[12:14], byteorder="little", signed=True) / 1000
        - zeroRef[5],
        2,
    )
...
if __name__ == "__main__":
    try:
        ...
        while True:
            ...
            # Data printing
            ###############
            print(f"F: {frequency}Hz - force Vector: {forceTorque}")
